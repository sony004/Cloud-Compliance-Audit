    ...
    def execute(self) -> list[CheckReportAlibabaCloud]:
        ...
        for sg_arn, security_group in ecs_client.security_groups.items():
            ...
            for ingress_rule in security_group.ingress_rules:
                # Normalize policy value to avoid case-sensitive mismatches
                # from providers returning "Accept" instead of "accept".
                policy = str(ingress_rule.get("policy", "accept")).strip().lower()
                if policy != "accept":
                    continue
